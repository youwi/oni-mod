...
namespace SingleStarWorldMod
{
    ...
    public static class SingleStarWorldModPatch
    {
        ...
        public static bool IsMyOneWorld()
        {
            ...
            ;
            Debug.Log("IsMyOneWorld: " + clName + " ->" + outFlag);
    ...
    public static class WorldGen_RenderOffline_Patch
    {
        ...
        public static void Postfix(WorldGen __instance, ref bool __result, ref Sim.Cell[] cells, ref Sim.DiseaseCell[] dc, int baseId)
        {
            ...
            Debug.Log($"RenderOffline:  GridSize: {Grid.HeightInCells} ,{Grid.WidthInCells}");
        }
    }
    ...
    public static class Cluster_Load_Patch
    {
        ...
        //    Debug.Log($"-----Cluster.Load Prefix Grid:{Grid.HeightInCells} ,{Grid.WidthInCells}");
        //    //  Debug.Log($"--LoadFromWorldGen size----->-- {__instance.}");
        ...
        public static void Postfix(Cluster __instance, ref Cluster __result)
        {
            ...
            Debug.Log($"-----Cluster.load Postfix Cluster.size----->-- {__result.size}");
            if (SingleStarWorldModPatch.IsMyOneWorld() || SingleStarWorldModPatch.IsMiniBase())
            {
                __result.size.x -= 99;
                Debug.Log($"-----修改的地图大小:----->-- {__result.size}");

            }


            //这个值可以改变大小
            Debug.Log($"-----Cluster.Load Postfix Grid.size:{Grid.HeightInCells} ,{Grid.WidthInCells}");
        }
    ...
    //            Debug.Log($"--->reset size:{width} ,{height}, GridSize: { Grid.HeightInCells} ,{Grid.WidthInCells}");
    //            Debug.Log($"--->WorldGen.GetSize:{__instance.GetSize()}");
    ...
    //        Debug.Log($"InitializeCells Grid:{Grid.HeightInCells} ,{Grid.WidthInCells}");
    ...
    //        Debug.Log($"----GridSettings.Reset :{width} ,{height}");
    //    }
    //}
    ...
    /* 
     ...
     public static class Cluster_BeginGeneration_Patch
     {
         public static void Postfix(Cluster __instance) 
         {
             ...
             {
                 Debug.Log($"----BeginGeneration size----->-- {wg.GetSize()}");
             }

             Debug.Log($"----Cluster_BeginGeneration_Patch:{Grid.HeightInCells} ,{Grid.WidthInCells}");
         }
         //this.m_clusterLayout
     }
     ...
     public static class SaveLoader_LoadFromWorldGen_Patch
     {
         public static void Prefix(SaveLoader __instance)
         {
             ...
             if(tt== null)
             {
                 Debug.Log($"-----LoadFromWorldGen Prefix:----->- null");
             }
             else
             {
                 Debug.Log($"-----LoadFromWorldGen Prefix:----->- {tt.size}");
             }

             Debug.Log($"-----LoadFromWorldGen Prefix:Grid:{Grid.HeightInCells} ,{Grid.WidthInCells}");
             //  Debug.Log($"--LoadFromWorldGen size----->-- {__instance.}");
             // Cluster.Save();
         ...
         public static void Postfix(SaveLoader __instance)
         {
             //生成之后查看世界大小
             //__instance.ClusterLayout.
             Debug.Log($"-----LoadFromWorldGen Postfix:----->-- {__instance.ClusterLayout.size}");

             Debug.Log($"-----LoadFromWorldGen Postfix Grid:{Grid.HeightInCells} ,{Grid.WidthInCells}");
         }
         //this.m_clusterLayout
     }
     //WORLDGEN COMPLETE
